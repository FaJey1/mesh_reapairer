...
class Mesh:
        ...
        with open(filename, "r") as f:
            ...
            while line:
                if line[0] == "#":
                    ...
                    right_varlocation_line = "VARLOCATION=([4-{0}]=CELLCENTERED)".format(len(variables))
                    if right_varlocation_line != varlocation_line[:-1]:
                        raise Exception(
                            "Wrong varlocation line ({0}). Right value is {1}".format(
                                varlocation_line, right_varlocation_line
                            )
                        )
    ...
    #     return self.find_edge(a, n), self.find_edge(b, n)

    # # ----------------------------------------------------------------------------------------------

    # def split_edge_power_2(self, e, n):
    #     """
    #     Split edge into 2^n parts.
    ...
    #     e1, e2 = self.split_edge(e)

    #     if n > 0:
    #         self.split_edge_power_2(e1, n - 1)
    #         self.split_edge_power_2(e2, n - 1)
